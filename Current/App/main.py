...
pjk_img = face_recognition.load_image_file(".\known_images\pjk.jpg")
pjk_encode = face_recognition.face_encodings(pjk_img)[0]

jay_img = face_recognition.load_image_file(".\known_images\jay.jpg")
...
known_face_id = [1,2]
known_emails = ["pranavthesteve@gmail.com", "pranavthesteve@gmail.com"]

absenties = known_face_names.copy()
absentiesId = known_face_id.copy()
absentiesMail = known_emails.copy()
        ...
        for face_encoding in face_encodings:
            ...
            if result in known_face_names and result in absenties:
                if int(now.strftime("%H")) < 2:
                    ...
                    absentiesMail.remove(resultMail)
                    print(absenties)
                    current_time = now.strftime("%H-%M-%S")
    ...
    if cv2.waitKey(1) & 0xFF == ord('q') or absenties == [] or cv2.getWindowProperty("Attendance system", cv2.WND_PROP_VISIBLE) < 1 :#or int(now.strftime("%H")) > 9 : 
        ...
        if(absenties != []):
            for i in range(len(absenties)):
                ...
                email.sendGmail("vnhtwpbrmjannhdv", "octocambot@gmail.com", absentiesMail[i], "Attendance regarding your son", f"This is to inform you that your son {result} is absent.")
        blynk.writeStr(pin, 0)
        print("------------------------------------------------------------------------------------")
        break